...
impl ScopeTracker {
    pub fn new() -> Self {
        // Initialize with a default scope to prevent panics when processing nodes
        // before any scope boundaries are encountered
        Self {
            scopes: vec![Scope {
    ...
    pub fn register<'a>(&mut self, node: tree_sitter::Node<'a>, position: usize) {
        ...
        // Should always have at least one scope since we initialize with a default scope
        // If this ever fails, it indicates a logic error in the scope tracking
        if let Some(scope) = self.scopes.last_mut() {
            scope.ancestors.register(node, position);
        }
    }
