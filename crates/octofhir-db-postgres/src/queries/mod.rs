//! SQL query modules for the PostgreSQL storage backend.
//!
//! This module contains the SQL query implementations organized by operation type.

pub mod crud;
pub mod history;
pub mod search;

// Re-export CRUD operations
pub use crud::{create, create_raw, delete, read, read_raw, update, update_raw, vread, vread_raw};

// Re-export history operations
pub use history::{get_history, get_history_raw, get_system_history, get_system_history_raw};

// Re-export search operations
pub use search::{
    SearchUnknownParamHandling, execute_search, execute_search_raw, execute_search_raw_with_config,
    execute_search_raw_with_terminology, execute_search_with_tx,
};
