...
export type FhirResourceClassification = "base" | "profile" | null;
...
export interface FhirPackageResourceView {
  ...
  urlLabel: string;
  versionLabel: string;
  /** For StructureDefinitions: whether it is a base resource/type or a profile. */
  classification: FhirResourceClassification;
  /** StructureDefinition.kind, when available (e.g. "resource", "complex-type"). */
  kindLabel?: string;
}

export function getFhirResourceClassification(
  resource: PackageResourceSummary
): FhirResourceClassification {
  if (resource.resourceType !== "StructureDefinition") return null;
  if (resource.derivation === "constraint") return "profile";
  if (resource.derivation === "specialization") return "base";
  return null;
}
...
export function filterFhirPackages(packages: PackageInfo[], search: string): PackageInfo[] {
  const query = search.trim().toLowerCase();
  if (!query) return packages;

  return packages.filter(
    (pkg) => pkg.name.toLowerCase().includes(query) || pkg.version.toLowerCase().includes(query)
  );
}
...
export function getFhirPackageInstalledViews(
  ...
  return packages.map((pkg) => ({
    ...
    installedAtLabel: pkg.installedAt ? new Date(pkg.installedAt).toLocaleDateString() : "-",
...
export function getFhirPackageResourceViews(
  ...
  return resources.map((resource, index) => ({
    ...
    classification: getFhirResourceClassification(resource),
    kindLabel: resource.kind || undefined,
  }));
}
