...
export function ensureFhirPathLanguageRegistered(): void {
  ...
  const alreadyRegistered = existingLanguages.some((l) => l.id === FHIRPATH_LANGUAGE_ID);
  ...
  monaco.languages.setMonarchTokensProvider(FHIRPATH_LANGUAGE_ID, {
    ...
    tokenizer: {
      root: [
        // Function calls: an identifier immediately followed by '(' (covers every
        // function, not just a fixed list). Keywords like `is`/`as` are never
        // before '(' so they fall through to the keyword rule below.
        [/[a-zA-Z_]\w*(?=\s*\()/, "predefined"],

        // Resource & complex types are PascalCase by FHIR convention
        // (Patient, Bundle, HumanName, Quantity). Highlight them distinctly.
        [/[A-Z]\w*/, "type"],

        // Keywords (and/or/is/as/…) and everything else = properties
        [
          /[a-zA-Z_]\w*/,
          {
            cases: {
 ...
 */
export async function startFhirPathLsp(getUrl: () => string): Promise<() => void> {
  if (activeConnection) {
 ...
 */
export function bindFhirPathModelToLsp(model: monaco.editor.ITextModel): () => void {
  if (!activeConnection) {
    return () => {};
  }
...
class FhirPathLspConnection {
  ...
  public dispose(): void {
    ...
    this.rejectAllPendingRequests(new Error("FHIRPath LSP connection disposed"));
  ...
  private async connect(): Promise<void> {
    ...
    socket.onclose = (event) => {
      logDebug(`[fhirpath-lsp] socket closed ${event.code} ${event.reason || ""}`.trim());
      this.initialized = false;
      this.socket = undefined;
      this.notificationQueue.length = 0;
      if (this.readyReject) {
        this.readyReject(new Error(`FHIRPath LSP connection closed before ready (${event.code})`));
      }
      this.readyPromise = null;
      this.readyResolve = undefined;
      this.readyReject = undefined;
      this.rejectAllPendingRequests(new Error(`FHIRPath LSP socket closed (${event.code})`));
  ...
  private handleMessage(data: string | ArrayBuffer | Blob): void {
    if (data instanceof Blob) {
      data
        ...
        .catch((error) => logDebug("[fhirpath-lsp] failed to read blob message", error));
      return;
    }
    ...
    if (typeof message.id !== "undefined") {
      ...
      if (message.error) {
        ...
        const msg = err.message ?? `FHIRPath LSP request ${pending.method} failed`;
  ...
  private async sendRequest<T>(
    ...
    return new Promise<T>((resolve, reject) => {
      const timeoutId = window.setTimeout(() => {
        ...
        reject(new Error(`FHIRPath LSP request ${method} timed out after 15s`));
      }, 15000);
  ...
  private registerLanguageFeatures(): monaco.IDisposable[] {
    ...
    const hoverProvider = monaco.languages.registerHoverProvider(FHIRPATH_LANGUAGE_ID, {
      provideHover: async (model, position) => {
        try {
          const response = await this.sendRequest<LspHover | null>("textDocument/hover", {
    ...
    const signatureProvider = monaco.languages.registerSignatureHelpProvider(FHIRPATH_LANGUAGE_ID, {
  ...
  private convertCompletionResponse(
    ...
    const suggestions = items.map((item) => this.toMonacoCompletion(item, model, position));

    const isIncomplete = Array.isArray(response) ? false : Boolean(response.isIncomplete);
...
function normalizeDocumentation(
  ...
  const normalized = documentation.kind === "markdown" ? value : ["```", value, "```"].join("\n");
  return { value: normalized, supportThemeIcons: true };
}

function normalizeHoverContents(contents: LspHover["contents"]): string {
  const toStr = (c: LspMarkedString | LspMarkupContent): string => {
    if (typeof c === "string") return c;
    if ("language" in c) return [`\`\`\`${c.language}`, c.value, "```"].join("\n");
    return c.value ?? "";
  };
  if (Array.isArray(contents)) {
    return contents.map(toStr).filter(Boolean).join("\n\n---\n\n");
  }
