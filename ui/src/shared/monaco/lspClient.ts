...
// Editor themes now live in the shared `./theme` module so every Monaco editor
// (not just FHIRPath/LSP) gets the same chrome + syntax colours. Re-exported
...
export function ensurePgLanguageRegistered(): void {
  ...
  const keywords = Array.from(new Set([...(sqlLanguage.keywords ?? []), ...PG_SPECIFIC_KEYWORDS]));
...
export function bindModelToLanguageServer(model: monaco.editor.ITextModel): () => void {
  if (!activeConnection) {
    return () => {};
  }
...
export function forceSyncModelToLanguageServer(model: monaco.editor.ITextModel): void {
  if (!activeConnection) {
    return;
  }
...
class PgLspConnection {
  ...
  private async connect(): Promise<void> {
    ...
    socket.onclose = (event) => {
      logDebug(`[pg-lsp] socket closed ${event.code} ${event.reason || ""}`.trim());
      this.initialized = false;
      this.socket = undefined;
      this.notificationQueue.length = 0;
      if (this.readyReject) {
        this.readyReject(new Error(`LSP connection closed before ready (${event.code})`));
      }
      this.readyPromise = null;
      this.readyResolve = undefined;
      this.readyReject = undefined;
      this.rejectAllPendingRequests(new Error(`LSP socket closed (${event.code})`));
  ...
  private handleMessage(data: string | ArrayBuffer | Blob): void {
    if (data instanceof Blob) {
      data
        ...
        .catch((error) => logDebug("[pg-lsp] failed to read blob message", error));
      return;
    }
    ...
    if (typeof message.id !== "undefined") {
      ...
      if (message.error) {
        const msg = message.error.message ?? `LSP request ${pending.method} failed`;
  ...
  private handleDiagnostics(params: LspPublishDiagnosticsParams): void {
    ...
    console.log(`[pg-lsp] Received ${diagnostics.length} diagnostics for ${uri}`, diagnostics);
    ...
    console.log(`[pg-lsp] Setting ${markers.length} markers on model ${uri}`, markers);
  ...
  private registerLanguageFeatures(): monaco.IDisposable[] {
    ...
    const completionProvider = monaco.languages.registerCompletionItemProvider(PG_LANGUAGE_ID, {
      triggerCharacters: [".", ">", ":", " ", "(", "'", '"', "{", ",", "$", "["],
    ...
    const hoverProvider = monaco.languages.registerHoverProvider(PG_LANGUAGE_ID, {
      provideHover: async (model, position) => {
        try {
          ...
          const hover = await this.sendRequest<LspHover>("textDocument/hover", params);
      ...
      PG_LANGUAGE_ID,
      {
        provideDocumentFormattingEdits: async (model, _options, _token) => {
          try {
            ...
            const edits = await this.sendRequest<LspTextEdit[]>("textDocument/formatting", params);
            if (!edits) return [];
...
function formatHoverContents(contents: unknown): string {
  ...
  if (typeof contents === "object" && "language" in contents && "value" in contents) {
