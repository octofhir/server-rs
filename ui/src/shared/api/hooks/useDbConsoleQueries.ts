...
import type { MaintenanceRequest, SaveHistoryRequest, TerminateQueryRequest } from "../types";
...
export const dbConsoleKeys = {
  ...
  tableDetail: (schema: string, table: string) =>
    [...dbConsoleKeys.all, "tableDetail", schema, table] as const,
...
export function useRunMaintenance() {
  ...
  return useMutation({
    ...
    }) => serverApi.runMaintenance(schema, table, request),
    onSuccess: (_data, { schema, table }) => {
      queryClient.invalidateQueries({ queryKey: dbConsoleKeys.tables() });
      queryClient.invalidateQueries({ queryKey: dbConsoleKeys.tableDetail(schema, table) });
    },
  });
}
