...
function QueryItem({
  ...
  return (
    <div className={classes.queryItem}>
      ...
      {query.query && (
        <Text size="xs" ff="monospace" className={classes.queryClamp2}>
          {query.query}
        </Text>
      )}
    </div>
  ...
  return (
    ...
    >
      <Tooltip label="Active queries">
        <UnstyledButton className={classes.activeQueriesTrigger}>
          <Pulse size={15} className={queries.length > 0 ? undefined : classes.mutedIcon} />
          {queries.length > 0 && (
            <Text size="xs" fw={600} c={activeCount > 0 ? "primary" : "dimmed"}>
              {queries.length}
            </Text>
          )}
        </UnstyledButton>
