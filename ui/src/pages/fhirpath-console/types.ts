...
function extractValue(param: {
  ...
  if (param.valueCodeableConcept !== undefined) return param.valueCodeableConcept;
...
export function parseParametersResponse(params: {
  ...
  const metadata: FhirPathMetadata = {
    ...
    timing: {
      parseTime: timingPart?.part?.find((p) => p.name === "parseTime")?.valueDecimal || 0,
      evaluationTime: timingPart?.part?.find((p) => p.name === "evaluationTime")?.valueDecimal || 0,
      totalTime: timingPart?.part?.find((p) => p.name === "totalTime")?.valueDecimal || 0,
    },
  };
