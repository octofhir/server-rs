...
export function useHistoryCommands(): ConsoleCommand[] {
	...
	return useMemo(() => {
		...
		return entries.slice(0, 20).map((entry) => {
			...
			const isSuccess =
				hasResponse && entry.responseStatus >= 200 && entry.responseStatus < 300;
			...
			else if (entry.responseStatus && entry.responseStatus >= 300)
				badgeColor = "yellow";

			// Build description with timing info
			const timeAgo = formatTimeAgo(entry.requestedAt);
			const duration = entry.responseDurationMs
			...
			return {
				...
				execute: (ctx) => {
					// Restore request from history with original mode
					// Default to "raw" for old entries that don't have mode saved
					ctx.setMode(entry.mode || "raw");
