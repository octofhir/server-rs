...
import { persistPreference } from "@/shared/api/userPreferences";
...
export const $mode = createStore<ConsoleMode>("pro").on(setMode, (_, mode) => mode);
...
export const $lastResponse = createStore<ConsoleResponseSnapshot | null>(null).on(
  setLastResponse,
  (_, response) => response
);
...
export const $defaultHeaders = createStore<Record<string, string>>(DEFAULT_FHIR_HEADERS).on(
  setDefaultHeaders,
  (_, headers) => headers
);
...
sample({
  ...
  clock: [setResourceType, setResourceId, setInteraction, setOperation, setSearchParams],
...
// `mode` is a genuine cross-browser preference → server-backed UserPreference.
// The remaining stores below are the transient request draft → kept in localStorage.
persistPreference({ store: $mode, key: "mode", namespace: "console" });
