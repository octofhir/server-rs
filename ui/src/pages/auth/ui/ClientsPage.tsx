        ...
        {isLoading ? (
          ...
          <EmptyState
            ...
            description={
              error instanceof Error
                ? error.message
                : "Something went wrong while loading OAuth clients."
            }
            actions={[
          ...
          <DataPreview
            ...
            getRowKey={(_row, index) =>
              clients[index]?.id ?? clients[index]?.clientId ?? `${index}`
            }
          />
        )}
      </Card>
...
function validateClientForm(values: ClientFormValues) {
  ...
  if (!values.clientId || values.clientId.length < 3)
