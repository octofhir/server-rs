...
import {
  ...
  Search as Magnifier,
  Pencil,
  UserPen as PersonPencil,
  UserX as PersonXmark,
  Plus,
  Trash2 as TrashBin,
} from "lucide-react";
      ...
      <Card variant="outlined" className={classes.tableContainer}>
        {someSelected && (
          <div className={classes.bulkActions}>
            ...
            <Button size="xs" variant="subtle" onClick={() => setSelectedUsers(new Set())}>
              Clear selection
            </Button>
          </div>
        )}
                ...
                users.map((user) => {
                  ...
                  return (
                    <Table.Tr key={user.id}>
                      ...
                      <Table.Td>
                        <div className={classes.roleList}>
                          {user.roles?.map((role) => {
                            ...
                            return (
                              <Badge key={role} size="sm" theme={roleView.theme}>
                                {roleView.role}
                              </Badge>
                            );
                          })}
                      ...
                      <Table.Td>
                        <Badge size="sm" theme={statusView.theme}>
                          {statusView.label}
                        </Badge>
                      </Table.Td>
                      <Table.Td>
...
function ResetPasswordModal({
  ...
  const validate = (values: ResetPasswordValues) => {
    ...
    else if (getPasswordStrength(values.newPassword).score < 2)
            ...
            <div className={classes.modalForm}>
              ...
              <div className={classes.modalActions}>
                ...
                <Button
                  variant="filled"
                  type="submit"
                  loading={submitting || resetPassword.isPending}
                >
                  Reset Password
                </Button>
              </div>
