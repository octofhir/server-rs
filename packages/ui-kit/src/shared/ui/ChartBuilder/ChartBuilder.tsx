...
const PALETTE_OPTIONS: { value: ChartPalette; label: string }[] = [
...
export function ChartBuilder({
  ...
  const toggleMeasure = (column: string, on: boolean) => {
    ...
    const numeric =
      column !== COUNT_FIELD.name && isNumericType(fieldByName.get(column)?.type ?? "text");
                      ...
                      <div key={f.name} className={styles.fieldRow}>
                        ...
                        {on && spec.type !== "scatter" && (
                          <Select
                            ...
                            value={ser?.agg ?? "count"}
                            onChange={(v) => setMeasureAgg(f.name, (v ?? "none") as Aggregation)}
                            size="sm"
                            w={88}
                          />
                        )}
          ...
          <Tabs.Panel value="style">
            <div className={styles.tabBody}>
              <div className={styles.switches}>
                ...
                <Switch
                  label="Data labels"
                  checked={spec.showLabels ?? false}
                  onChange={(v) => update({ ...spec, showLabels: v })}
                  size="sm"
                />
                {spec.type === "pie" && (
                  <Switch
                    label="Donut"
                    checked={spec.donut ?? false}
                    onChange={(v) => update({ ...spec, donut: v })}
                    size="sm"
                  />
                )}
              </div>
              ...
              <div className={styles.field}>
                ...
                <Select
                  ...
                  value={spec.palette ?? "theme"}
                  onChange={(v) => update({ ...spec, palette: (v ?? "theme") as ChartPalette })}
                  size="sm"
                />
              </div>
